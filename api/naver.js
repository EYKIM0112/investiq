...
export default async function handler(req, res) {
  ...
  if (type === "search") {
    ...
    try {
      ...
      if (r.ok) {
        ...
        const matches = [...html.matchAll(/href="\/item\/main\.naver\?code=(\d{4}[0-9A-Z]{2})"[^>]*>([^<]{2,50})<\/a>/g)];
        if (matches.length > 0) {
          const seen = new Set();
          const items = [];
      ...
      try {
        ...
        if (gRes.ok) {
          ...
          if (jsonMatch) {
            ...
            const quotes = items
              .filter(i => i.code && /^\d{4}[0-9A-Z]{2}$/.test(String(i.code)))
              .map(i => {
  ...
  if (type === "names") {
    ...
    const codeList = codes.split(",").map(c => c.trim()).filter(c => /^\d{4}[0-9A-Z]{2}$/.test(c)).slice(0, 10);
